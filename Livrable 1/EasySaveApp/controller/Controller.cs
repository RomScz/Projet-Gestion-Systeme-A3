...
namespace EasySaveApp.controller
{
    class Controller
    {
        ...
        public Controller()
        {
            model = new Model();
            view = new View();
            view.ViewStart(); //Function call
            model.userMenuInput = Menu(); //Function call

        }

        static void ReadFile(string cheminFichier)
        {
            StreamReader Read = new StreamReader(cheminFichier);
            string line;
        ...
        private string GetSourceDir() //Function to retrieve the input from the source
        {
            ...
            while (!isValid) //Loop to allow verification of the path
            {
                ...
                {
                    view.MenuError("Incorect Path"); //Show error message
                }
        ...
        private string Menu() //function for menu management
        {
            ...
            while (menu) //Loop for menu
            {
                ...
                try
                {
                    view.ViewMenu(); //Calling the function to display the menu
                    ...
                    switch (inputMenu) // Switch of menu
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            view.ViewFileName(); //Display message introduction on the backup names
                            ...
                            view.ViewFile();//Calling the function to display the names of the backups
                        ...
                        case 2:
                            if (model.checkdatabackup < 5) // Check not to exceed the save limit
                            {
                                ...
                                view.ViewBackupMenu(); // Calling the function to display the second menu
                                MenuSub(); // Calling the function for the second menu
                            }
                            else
                            {
                                Console.Clear(); //Console cleaning
                                view.MenuError("You already have 5 backups to create."); // Show Error Message
                            }
                ...
                try
                {
                    ...
                    switch (inputMenuSub) // Switch of menu
                    {
                        ...
                        case 1: //Case 1, creating a full backup job
                            ...
                            view.EnterName(); //Display for backup name
                            model.SaveName = Console.ReadLine(); // Retrieving the name of the backup
                            view.EnterSourceFold(); // Display for folder source
                            model.SourceDir = GetSourceDir(); // Function for checking the folder path
                            view.EnterTargetFold(); // Display for the folder destination
                        ...
                        case 2: //Case 2, creating a differential backup job
                            ...
                            view.EnterSourceFold();
                            model.SourceDir = GetSourceDir();
                            view.EnterMirrorFold();
                            model.MirrorDir = GetMirrorDir();
                            view.EnterTargetFold();
